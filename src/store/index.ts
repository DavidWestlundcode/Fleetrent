...
function fromDbMachine(r: DbRow): Machine {
  return {
    ...
    mastType: (r.mast_type as string) ?? undefined,
    powerUnit: (r.power_unit as string) ?? undefined,
    cabin: (r.cabin as string) ?? undefined,
...
function toDbMachine(m: Machine, orgId: string): DbRow {
  return {
    ...
    mast_type: m.mastType ?? null,
    power_unit: m.powerUnit ?? null,
    cabin: m.cabin ?? null,
    year: m.year,
...
function fromDbCustomer(r: DbRow): Customer {
  return {
    ...
    facilities: (r.facilities as CustomerFacility[]) ?? [],
    spMachines: (r.sp_machines as SpMachine[] | null) ?? undefined,
    isActive: (r.is_active as boolean) ?? true,
    createdAt: r.created_at as string,
    spId: (r.sp_id as string) || undefined,
  };
}
...
function fromDbOrder(r: DbRow): Order {
  ...
  return {
    ...
    facilityName: (r.facility_name as string) || undefined,
    ordererName: (r.orderer_name as string) || undefined,
    ordererPhone: (r.orderer_phone as string) || undefined,
    ordererEmail: (r.orderer_email as string) || undefined,
...
function toDbOrder(o: Order, orgId: string): DbRow {
  return {
    ...
    facility_name: o.facilityName ?? null,
    orderer_name: o.ordererName ?? null,
    orderer_phone: o.ordererPhone ?? null,
    orderer_email: o.ordererEmail ?? null,
...
function fromDbTemplate(r: DbRow): PriceTemplate {
  return {
    ...
    dailyPrice: (r.daily_price as number) ?? 0,
    dailyPriceDiscount: (r.daily_price_discount as number) ?? 0,
    weeklyPrice: (r.weekly_price as number) ?? 0,
    weeklyPriceDiscount: (r.weekly_price_discount as number) ?? 0,
    monthlyPrice: (r.monthly_price as number) ?? 0,
    monthlyPriceDiscount: (r.monthly_price_discount as number) ?? 0,
    longTermDailyPrice: (r.long_term_daily_price as number) ?? 0,
    longTermDailyDiscount: (r.long_term_daily_discount as number) ?? 0,
    longTermWeeklyPrice: (r.long_term_weekly_price as number) ?? 0,
    longTermWeeklyDiscount: (r.long_term_weekly_discount as number) ?? 0,
    longTermMonthlyPrice: (r.long_term_monthly_price as number) ?? 0,
    longTermMonthlyDiscount: (r.long_term_monthly_discount as number) ?? 0,
    insuranceDailyPrice: (r.insurance_daily_price as number) ?? 0,
    insuranceDailyDiscount: (r.insurance_daily_discount as number) ?? 0,
    insuranceWeeklyPrice: (r.insurance_weekly_price as number) ?? 0,
    insuranceWeeklyDiscount: (r.insurance_weekly_discount as number) ?? 0,
    insuranceMonthlyPrice: (r.insurance_monthly_price as number) ?? 0,
    insuranceMonthlyDiscount: (r.insurance_monthly_discount as number) ?? 0,
    startFee: (r.start_fee as number) ?? 0,
    startFeeDiscount: (r.start_fee_discount as number) ?? 0,
    transportCost: (r.transport_cost as number) ?? 0,
    transportDiscount: (r.transport_discount as number) ?? 0,
    deposit: (r.deposit as number) ?? 0,
    ...
    customerType: (r.customer_type as PriceTemplate['customerType']) ?? 'alla',
    createdAt: r.created_at as string,
  };
}
...
function toDbTemplate(t: PriceTemplate, orgId: string): DbRow {
  return {
    ...
    daily_price_discount: t.dailyPriceDiscount ?? 0,
    weekly_price: t.weeklyPrice,
    weekly_price_discount: t.weeklyPriceDiscount ?? 0,
    monthly_price: t.monthlyPrice,
    monthly_price_discount: t.monthlyPriceDiscount ?? 0,
    long_term_daily_price: t.longTermDailyPrice,
    long_term_daily_discount: t.longTermDailyDiscount ?? 0,
    long_term_weekly_price: t.longTermWeeklyPrice,
    long_term_weekly_discount: t.longTermWeeklyDiscount ?? 0,
    long_term_monthly_price: t.longTermMonthlyPrice,
    long_term_monthly_discount: t.longTermMonthlyDiscount ?? 0,
    insurance_daily_price: t.insuranceDailyPrice,
    insurance_daily_discount: t.insuranceDailyDiscount ?? 0,
    insurance_weekly_price: t.insuranceWeeklyPrice,
    insurance_weekly_discount: t.insuranceWeeklyDiscount ?? 0,
    insurance_monthly_price: t.insuranceMonthlyPrice,
    insurance_monthly_discount: t.insuranceMonthlyDiscount ?? 0,
    start_fee: t.startFee,
    start_fee_discount: t.startFeeDiscount ?? 0,
    transport_cost: t.transportCost,
    transport_discount: t.transportDiscount ?? 0,
    deposit: t.deposit,
    ...
    customer_type: t.customerType ?? 'alla',
...
export const useStore = create<AppStore>()((set, get) => ({
  ...
  editInvoicePeriod: (orderId, periodId, updates) => {
    const now = new Date().toISOString();
    const order = get().orders.find((o) => o.id === orderId);
    if (!order) return;
    const period = (order.invoicePeriods ?? []).find((p) => p.id === periodId);
    if (!period || period.sentToAccounting) return;

    const deltaDays = updates.days - period.days;
    const deltaAmount = updates.amount - period.amount;

    // A period generated by a machine swap booked its revenue against the OUTGOING machine —
    // correct that machine's stats, not whatever machine is currently on the order.
    const swap = (order.machineSwaps ?? []).find((s) => s.invoicePeriodId === periodId);
    const targetMachineId = swap?.fromMachineId ?? order.machineId;
    ...
    set((s) => ({
      ...
      machines: s.machines.map((m) =>
        m.id === targetMachineId
          ? { ...m, totalRevenue: m.totalRevenue + deltaAmount, totalRentalDays: m.totalRentalDays + deltaDays, updatedAt: now }
          : m
      ),
      customers: s.customers.map((c) =>
        c.id === order.customerId ? { ...c, totalSpent: c.totalSpent + deltaAmount } : c
      ),
    }));
    ...
    if (organizationId) {
      const updatedOrder = get().orders.find((o) => o.id === orderId);
      const updatedMachine = get().machines.find((m) => m.id === targetMachineId);
      const updatedCustomer = get().customers.find((c) => c.id === order.customerId);
      const client = sb();
      Promise.all([
        updatedOrder ? client.from('orders').update({ invoice_periods: updatedOrder.invoicePeriods ?? [] }).eq('id', orderId) : Promise.resolve({ error: null }),
        updatedMachine ? client.from('machines').update({ total_revenue: updatedMachine.totalRevenue, total_rental_days: updatedMachine.totalRentalDays, updated_at: now }).eq('id', targetMachineId) : Promise.resolve({ error: null }),
        updatedCustomer ? client.from('customers').update({ total_spent: updatedCustomer.totalSpent }).eq('id', order.customerId) : Promise.resolve({ error: null }),
      ]).then((results) => {
        results.forEach((r, i) => { if (r.error) console.error(`sync editInvoicePeriod[${i}]:`, r.error); });
      });
    }
  },
  ...
  updateOrder: (id, updates) => {
    ...
    if (organizationId) {
      ...
      if (updates.invoicePeriods !== undefined) dbUpdates.invoice_periods = updates.invoicePeriods;
