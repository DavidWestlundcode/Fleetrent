                          ...
                          <div className="text-sm text-emerald-700 font-medium flex flex-col gap-1">
                            ...
                            {spSyncResult.debug && (
                              <span className="text-xs text-slate-500 font-mono break-all whitespace-pre-wrap">
                                {`kundnr=${String(spSyncResult.debug.spCustomerNo ?? '–')} | ${String(spSyncResult.debug.rawCount ?? 0)} objekt | spId="${String(spSyncResult.debug.firstSpId ?? '–')}" | namn="${String(spSyncResult.debug.firstName ?? '–')}" | hoppade(ingetId=${String(spSyncResult.debug.skippedNoId ?? 0)} finns=${String(spSyncResult.debug.skippedExists ?? 0)}) | borttagna=${String(spSyncResult.debug.removedMachines ?? 0)}\nCustomInfo: ${JSON.stringify(spSyncResult.debug.firstCustomInfo ?? null).slice(0, 400)}\nÅrsfält: ${JSON.stringify(spSyncResult.debug.yearFields ?? {})}\nParsed: ${JSON.stringify(spSyncResult.debug.firstParsedSpecs ?? {})}\nRå (2000 tecken): ${String(spSyncResult.debug.firstRawFull ?? '(saknas)')}${spSyncResult.debug.diagStatus !== undefined ? `\nDiag med kundnr HTTP: ${String(spSyncResult.debug.diagStatus)}\nDiag med kundnr svar: ${String(spSyncResult.debug.diagBody ?? '')}\nDiag utan kundnr HTTP: ${String(spSyncResult.debug.diagBaseStatus ?? '–')}\nDiag utan kundnr svar: ${String(spSyncResult.debug.diagBaseBody ?? '')}` : ''}${spSyncResult.debug.error ? `\nFEL: ${String(spSyncResult.debug.error)}` : ''}`}
                              </span>
                            )}
                          </div>
                        )}
